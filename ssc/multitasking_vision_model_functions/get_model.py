...
from ssc.multitasking_vision_model_functions.lsrapp import ConvBnRelu
...
def get_model(args, tasks_outputs, num_inp_feats=3):
	...
	if args.backbone == 'segnet':
		from ssc.multitasking_vision_model_functions.segnet import SegNet
		backbone = SegNet(num_inp_feats=num_inp_feats)
		backbone_channels = 64
	elif args.backbone == "mobilenetv3":
		from ssc.multitasking_vision_model_functions.lsrapp import LRASPP
		...
		from ssc.multitasking_vision_model_functions.deeplabv3 import DeepLabV3Plus
		backbone_channels = 256
		backbone = DeepLabV3Plus(decoder_channels=backbone_channels, in_channels=num_inp_feats)
	# TODO: more backbones

	if args.method == 'single-task':
		from ssc.multitasking_vision_model_functions.models import SingleTaskModel
		task = args.task
...
def get_head(head, backbone_channels, task_output):
	...
	if head == 'segnet_head':
		from ssc.multitasking_vision_model_functions.segnet import SegNet_head
