...
# Local imports
import  ssc.preprocessing_functions.ssc_preprocessing_functions.deepwatermap as deepwatermap
...
def get_hls_links(json_data, site_id, date, fail_log_fp):
    try:
...
def cv_preprocessing(indir:str, all_tiles_in_memory:list, save_masks_bool:bool, max_thresh_for_multitasking_vision_model_preprocessing:int):
    ...
    MODEL_CKPT_PATH = os.path.join('opt', 'models', 'model_static_files', 'rangel_checks', 'multitask_ckpts', 'deeplabv3p.pth.tar')
    ...
    geotran= all_tiles_in_memory[0].read_transform()
    projection_reproduce = all_tiles_in_memory[0].read_crs()
    print(projection_reproduce)

    # #get shape of array
    # getshape_1 = all_tiles_in_memory[0].GetRasterBand(1)
    # getshape_2 = getshape_1.ReadAsArray()
    size1,size2=all_tiles_in_memory[0].read()[0].shape
    ...
    # start_time = tm.time()
    water_mask, pred_time = get_water_mask(model_path=MODEL_CKPT_PATH, image=all_tiles_in_memory, out_dir='.',
                                            mask_thresh=max_thresh_for_multitasking_vision_model_preprocessing)
